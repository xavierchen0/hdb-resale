...
# %%
df_other_loc = gpd.read_parquet(DATA_DIR / "loc_jc_sch.parquet")
...
hdb_nearest = df.sjoin_nearest(
    ...
    how="left",
    distance_col="distance_to_nearest_jcschool",  # Name the output distance column
)
hdb_nearest
...
# %%
final_df[["num_jcschools_500m", "num_jcschools_1000m"]] = (
    final_df[["num_jcschools_500m", "num_jcschools_1000m"]].fillna(0).astype(int)
)
final_df

# %%
jcschool_points = df_other_loc.geometry.to_list()
jcschool_array = np.array([(p.x, p.y) for p in jcschool_points])

final_df["jcschool_access"] = final_df.geometry.apply(
    lambda geom: accessibility_score_one_point(geom, jcschool_array, lam=500)
)
final_df
...
# %%
df_merged.to_parquet(DATA_DIR / "feat" / "feat_loc_jcschool.parquet")
