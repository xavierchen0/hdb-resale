...
hdb_nearest = df.sjoin_nearest(
    ...
    how="left",
    distance_col="distance_to_nearest_mixedschool",  # Name the output distance column
)
hdb_nearest
...
# %%
final_df[["num_mixedschools_500m", "num_mixedschools_1000m"]] = (
    final_df[["num_mixedschools_500m", "num_mixedschools_1000m"]].fillna(0).astype(int)
)
final_df

# %%
mixedschool_points = df_other_loc.geometry.to_list()
mixedschool_array = np.array([(p.x, p.y) for p in mixedschool_points])

final_df["mixedschool_access"] = final_df.geometry.apply(
...
df_merged = final_df.loc[
    ...
    [
        "txn_id",
        "distance_to_nearest_mixedschool",
        "num_mixedschools_500m",
        "num_mixedschools_1000m",
        "mixedschool_access",
    ],
]
df_merged
...
# %%
df_merged.to_parquet(DATA_DIR / "feat" / "feat_loc_mixedschool.parquet")
