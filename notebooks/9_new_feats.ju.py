...
df_coe1 = (
    df_coe[(df_coe["month"] >= START_DATE) & (df_coe["month"] <= END_DATE)]
    .copy()
    .reset_index()
)
df_coe1
...
# merge the df
df_merged1 = pd.merge(df, df_coe_pivot, on="month", how="left")
...
][["month", "cpi_pct_change", "cpi_index"]].copy()
...
new_row = pd.DataFrame(
    {"month": [END_DATE], "cpi_index": [last_cpi_value], "cpi_pct_change": 0}
)
...
# %%
# merge
df_merged2 = pd.merge(df_merged1, df_cpi_final, on="month", how="left")
...
df_ppi_filtered = df_ppi[
    (df_ppi["quarter"] >= "2017-Q1") & (df_ppi["quarter"] <= "2025-Q3")
].copy()
...
df_ppi_filtered["month_start"] = df_ppi_filtered["quarter"].apply(
    convert_quarter_to_date
)
df_ppi_filtered
...
# merge the df
df_merged3 = pd.merge(df_merged2, df_ppi_monthly, on="month", how="left")
...
df_ue_long = df_ue.melt(
    id_vars=["DataSeries"], var_name="month_str", value_name="unemployment_rate"
)
...
df_ue_all = df_ue_long[
    df_ue_long["DataSeries"] == "Total Unemployment Rate, (SA)"
].copy()
...
df_ue_filtered = df_ue_all[
    (df_ue_all["month_start"] >= "2017-Q1") & (df_ue_all["month_start"] <= "2025-Q3")
].copy()
...
df_ue_monthly.rename(
    columns={"unemployment_rate": "unemployment_rate_sa"}, inplace=True
)
...
# %%
# merge the df
df_merged4 = pd.merge(df_merged3, df_ue_to_merge, on="month", how="left")
...
df_hi_long = df_hi_filtered.melt(
    id_vars="PerCent", var_name="Year", value_name="50th (Median)"
)
...
df_hi_filtered = df_hi_long[
    (df_hi_long["Year"] >= "2017-Q1") & (df_hi_long["Year"] <= "2025-Q3")
].copy()
...
df_hi_monthly.loc[df_hi_monthly.index >= start_of_2025, "income_change"] = (
    ANNUALISED_VALUE
)
...
# %%
# merge the df
df_merged5 = pd.merge(df_merged4, df_hi_monthly, on="month", how="left")
